...
//FIXME boczne separatory, nie działają bo nie wiem jak zaimportować jednostki
...
QtObject {
    id: root
//    readonly property HPSUnits units: HPSUnits

    readonly property Component separator: Rectangle {
        property Item column
        implicitHeight: 1
        color: "#212121"

        visible: column.HPSColumnView.view && column.HPSColumnView.view.contentX < column.x
        anchors.top: column.top
        anchors.bottom: column.bottom
    }

    readonly property Component rightSeparator: Rectangle {
        property Item column
        implicitHeight: 1
        color: "#212121"
...
)"), QUrl(QStringLiteral("HPSColumnView.cpp")));
...
//    m_units = engine->singletonInstance<HPSUnits>(qmlTypeId("HPSUnits", 2, 0, "HPSUnits"));
//    Q_ASSERT(m_units);

    // connect(m_units, &HPSUnits::gridUnitChanged, this, &QmlComponentsPool::gridUnitChanged);
    // connect(m_units, &HPSUnits::longDurationChanged, this, &QmlComponentsPool::longDurationChanged);
}
...
void ColumnView::classBegin()
{
    ...
    auto syncColumnWidth = [this]() {
        m_contentItem->m_columnWidth = 20 * 20;
        Q_EMIT columnWidthChanged();
    };
    ...
    auto syncDuration = [this]() {
        m_contentItem->m_slideAnim->setDuration(200);
        Q_EMIT scrollDurationChanged();
    };
